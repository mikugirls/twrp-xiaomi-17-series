...
* After flashing a ROM update newer than the recovery's own donor firmware
  ...
  mismatches for the dynamic super partitions. Use a recovery built against a
  matching or newer stock recovery donor and an EROFS-only fstab for the
  dynamic partitions to avoid the noise.
